...
export default function YearWiseGP(props:any) {
   const { headerTitle} = props;
    ...
    const [colDef] = useState<ColDef<any>[]>([
        ...
        {
            field: "SALE_VALUE22", headerName: "Sales GP 2022", cellClass: "text-right", flex: 1, valueFormatter: (params) => {
                if (params.value == null) return "";
                return params.value.toLocaleString();
            }
        },
        {
            field: "SALE_VALUE23", headerName: "Sales GP 2023", cellClass: "text-right", flex: 1, valueFormatter: (params) => {
                if (params.value == null) return "";
                return params.value.toLocaleString();
            }
        },
        {
            field: "SALE_VALUE24", headerName: "Sales GP 2024", cellClass: "text-right", flex: 1, valueFormatter: (params) => {
                if (params.value == null) return "";
                return params.value.toLocaleString();
            }
        },
        {
            field: "SALE_VALUE25", headerName:"Sales GP 2025", cellClass: "text-right", flex: 1, valueFormatter: (params) => {
                if (params.value == null) return "";
                return params.value.toLocaleString();
            }
        },
    ...
    useEffect(() => {
        ...
        // fetch(`${import.meta.env.VITE_API_BACKEND_URL}/api/year-wise-gp-comparison?yyyymm=${yyyymm}&location=${selectedStore?.LOCATION_ID}`)
        fetch(`/api/year-wise-gp-comparison.json`)
    ...
    const calculateTotals = (data: any[]) => {
        ...
        const numericCols = ["SALE_VALUE22", "SALE_VALUE23", "SALE_VALUE24", "SALE_VALUE25",]
    ...
    useEffect(() => {
        ...
        const newSeries = [
            {
                name: "Sales GP 2022",
                data: source.map(item => item.SALE_VALUE22 ?? 0)
            },
            {
                name: "Sales GP 2023",
                data: source.map(item => item.SALE_VALUE23 ?? 0)
            },
            {
                name: "Sales GP 2024",
                data: source.map(item => item.SALE_VALUE24 ?? 0)
            },
            {
                name: "Sales GP 2025",
                data: source.map(item => item.SALE_VALUE25 ?? 0)
            }
        ];
