...
export default function Fastline(props: any) {
    ...
    const [colDef] = useState<ColDef<any>[]>([
        { field: "SECTION_NAME", headerName: "Name", cellClass: "text-left ", flex: 1 },

        { field: "SECTION_CODE", headerName: "Code", cellClass: "text-right", flex: 1 },
        {
        ...
        {
            field: "STOCK_SKU", headerName: "Stock SKU", cellClass: "text-right", flex: 1, valueFormatter: (params) => {
                if (params.value == null) return "";
                return params.value.toLocaleString();
            },
        },
        {
            field: "NEG_STOCK", headerName: "Neg. Stock", cellClass: "text-right", flex: 1, valueFormatter: (params) => {
                if (params.value == null) return "";
                return params.value.toLocaleString();
            }
        },
        {
            field: "TOTAL_OOS", headerName: "Total OOS", cellClass: "text-right", flex: 1, valueFormatter: (params) => {
                if (params.value == null) return "";
                return params.value.toLocaleString();
            }
        },
        ...
        {
            field: "WH_STOCK", headerName: "WH Stock", cellClass: "text-right", flex: 1, valueFormatter: (params) => {
                if (params.value == null) return "";
                return params.value.toLocaleString();
            }
        },
        {
            field: "WH_STOCK_NOTIN_STORE", headerName: "WH Stk ,Not In Stock", cellClass: "text-right", flex: 1, valueFormatter: (params) => {
                if (params.value == null) return "";
                return params.value.toLocaleString();
            }
        },
    ...
    useEffect(() => {
        ...
        // fetch(`${import.meta.env.VITE_API_BACKEND_URL}/api/fast-line?yyyymm=${yyyymm}&location=${selectedStore?.LOCATION_ID}`)
        fetch(`/api/fast-line.json`)
    ...
    useEffect(() => {
        ...
        const AVLB_ITEM_PERC = source.map((item) => (item.STOCK_SKU / item.SKU_COUNT * 100 || 0), 0);
        const OOS_PERC = source.map((item) => (item.TOTAL_OOS / item.SKU_COUNT * 100 || 0), 0);
        ...
        const newSeries = [
            ...
            {
                name: "Available SKU (%)",
                data: AVLB_ITEM_PERC.map(num => num.toFixed(3))
            },
            {
                name: "OOS SKU (%)",
                data: OOS_PERC.map(num => num.toFixed(3))
            },
        ];
